...
// constants.
import * as likesAbi from "./abi/likes";
// UsdcTransfer is a TypeORM model class of the usdc_transfer Postgres table.
// It is autogenerated from schema.graphql.
import { LikesTransfer } from "./model";

const LIKES_CONTRACT_ADDRESS = "0x403BfD53617555295347e0F7725CfdA480AB801e";
...
const processor = new EvmBatchProcessor()
  ...
  .addLog({
    range: { from: 4622902 },
    address: [LIKES_CONTRACT_ADDRESS],
    topic0: [likesAbi.events.Transfer.topic],
  })
...
processor.run(db, async (ctx) => {
  ...
  for (let block of ctx.blocks) {
    ...
    for (let log of block.logs) {
      if (
        log.address === LIKES_CONTRACT_ADDRESS.toLowerCase() &&
        log.topics[0] === likesAbi.events.Transfer.topic
      ) {
        // SQD's very own EVM codec at work - about 20 times faster than ethers
        let { from, to, amount } = likesAbi.events.Transfer.decode(log);
        transfers.push(
          new LikesTransfer({
            id: log.id,
