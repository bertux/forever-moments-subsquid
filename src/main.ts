...
// constants.
import * as luksoAbi from "./abi/lukso";
...
const LUKSO_CONTRACT_ADDRESS = "0x403BfD53617555295347e0F7725CfdA480AB801e";
...
const processor = new EvmBatchProcessor()
  ...
  .addLog({
    range: { from: 4622902 },
    address: [LUKSO_CONTRACT_ADDRESS],
    topic0: [luksoAbi.events.Transfer.topic],
  })
...
processor.run(db, async (ctx) => {
  ...
  for (let block of ctx.blocks) {
    ...
    for (let log of block.logs) {
      if (
        log.address === LUKSO_CONTRACT_ADDRESS.toLowerCase() &&
        log.topics[0] === luksoAbi.events.Transfer.topic
      ) {
        // SQD's very own EVM codec at work - about 20 times faster than ethers
        let { from, to, amount } = luksoAbi.events.Transfer.decode(log);
        transfers.push(
          new LikesTransfer({
            id: log.id,
